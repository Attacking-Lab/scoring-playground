...
def parse_args(args: list[str]):
    ...
    parser.add_argument(
        "--data",
        help="Selects the CTF data source",
        choices=data_sources,
        required=True,
    )
    parser.add_argument(
        "--formula",
        help="Selects the scoring formula",
        choices=scoring_formulas,
        required=True,
    )
    parser.add_argument(
        "--output-format",
    ...
    parser.add_argument(
        "--from-round",
        help="Assume the CTF started in this round (this round is included in scoring)",
        type=int,
    )
    parser.add_argument(
        "--to-round",
        help="Assume the CTF ended in this round (this round is included in scoring)",
        type=int,
    )
    parser.add_argument(
        "--scale-to",
        help="Scale the final scoreboard to this maximum point count",
        type=float,
    )

    config_parser = argparse.ArgumentParser(
        add_help=False, allow_abbrev=False, usage=argparse.SUPPRESS
    )

    def base_type_of(
        type_hint: typing.Any, *, location: str | None = None
    ) -> tuple[type, bool]:  # (type, allowed to be none?)
        if type_hint is dataclasses.MISSING:
            warnings.warn(
                "Missing type hint"
                + (f" for {location}" if location is not None else "")
                + ", falling back to str"
            )
            return str, False
            ...
            if len(union_types) > 1:
                warnings.warn(
                    f"Cannot handle multi-type union {union_types}"
                    + (f" for {location}" if location is not None else "")
                    + ", falling back to str"
                )
            ...
            base_type, base_may_be_none = base_type_of(
                union_types.pop(), location=location
            )
            ...
            warnings.warn(
                f"Cannot handle type hint {type_hint}"
                + (f" for {location}" if location is not None else "")
                + ", falling back to str"
            )
            return str, False
    ...
    def build_options_parser(root: argparse.ArgumentParser, title: str, ty: type):
        ...
        fields = [
            field for field in dataclasses.fields(ty) if not field.name.startswith("_")
        ]
        if not fields:
            return
        ...
        for field in fields:
            ...
            argument_type, may_be_none = base_type_of(
                expected_type, location=f"{ty.__name__}.{field.name}"
            )
            ...
            if issubclass(argument_type, enum.Enum):
                ...
                argument_type = (
                    lambda key, enum_type=argument_type: enum_type[key]
    ...
    if help_args.help:
        source = next(
            (source for source in sources if source.__name__ == help_args.data), None
        )
        formula = next(
            (formula for formula in formulas if formula.__name__ == help_args.formula),
            None,
        )
        ...
        case "table":
            ...
            data = [
                ...
                for team, score in sorted(
                    scoreboard.items(), key=lambda item: item[1].combined, reverse=True
                )
            ]
            if base.scale_to:
                keys.append("(scaled)")
            ...
            print(
                tabulate.tabulate(
                    data, headers=("Team", "Score", *keys), tablefmt="simple_grid"
                )
            )
